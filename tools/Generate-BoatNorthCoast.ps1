...
$generationRoot = Join-Path $buildRoot "boat-north-coast-generation"
$stagingData = Join-Path $generationRoot "data"
$pluginsList = Join-Path $generationRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-north-coast.status"
$errorPath = Join-Path $buildRoot "boat-north-coast.error"
$scriptPath = Join-Path $PSScriptRoot "xedit\DNT_GenerateBoatNorthCoast.pas"
$pluginOutput = Join-Path $modRoot "DiegeticTravelBoatNorthCoast.esp"
$seqOutput = Join-Path $modRoot "SEQ\DiegeticTravelBoatNorthCoast.seq"
$seqFormIdsPath = Join-Path $buildRoot "boat-north-coast-seq-formids.txt"
...
if (-not $resolvedGenerationRoot.StartsWith(
    ...
    throw "Refusing to clean North-coast staging outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required North-coast generation input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required North-coast generation input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "North-coast generation failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "North-coast generation did not report success."
}
foreach ($outputPath in @($pluginOutput, $seqFormIdsPath)) {
    if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) {
        throw "North-coast generation did not create: $outputPath"
    }
}
...
if ($seqFormIds.Count -ne 1 -or $seqFormIds[0] -notmatch "^[0-9A-Fa-f]{8}$") {
    throw "North-coast generator did not report exactly one valid SEQ FormID."
}
...
Write-Host "Generated North-coast boat candidate: $pluginOutput"
Write-Host "Generated North-coast boat SEQ: $seqOutput ($($seqFormIds[0]))"
