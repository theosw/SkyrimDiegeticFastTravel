...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required North-coast Papyrus input not found: $requiredPath"
    }
}
...
$scripts = Get-ChildItem -LiteralPath $source -File `
    -Filter "DNT_NorthCoastBoat*.psc" | Sort-Object Name
if ($scripts.Count -ne 3) {
    throw "Expected exactly three North-coast scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) North-coast scripts to $output"
