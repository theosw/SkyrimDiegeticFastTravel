param(
    ...
    [string]$PackageName = "DiegeticTravelBoatNorthCoast-offline-candidate",
    [switch]$PackageOnly
)
...
if (-not $PackageOnly) {
    & (Join-Path $PSScriptRoot "Compile-BoatNorthCoastPapyrus.ps1") `
        -LoreRimRoot $LoreRimRoot
    & (Join-Path $PSScriptRoot "Generate-BoatNorthCoast.ps1") `
        -LoreRimRoot $LoreRimRoot `
        -XEdit $XEdit `
        -SkipCompile
    & (Join-Path $PSScriptRoot "Audit-BoatNorthCoast.ps1") `
        -LoreRimRoot $LoreRimRoot `
        -XEdit $XEdit
...
$requiredInputs = @(
    (Join-Path $modRoot "DiegeticTravelBoatNorthCoast.esp"),
    (Join-Path $modRoot "README-BoatNorthCoast.txt"),
    (Join-Path $modRoot "SEQ\DiegeticTravelBoatNorthCoast.seq"),
    (Join-Path $modRoot "Scripts\DNT_NorthCoastBoatTravelService.pex"),
    (Join-Path $modRoot "Scripts\DNT_NorthCoastBoatParchmentPicker.pex"),
    (Join-Path $modRoot "Scripts\DNT_NorthCoastBoatParchmentFragment.pex"),
    (Join-Path $modRoot "Scripts\Source\DNT_NorthCoastBoatTravelService.psc"),
    (Join-Path $modRoot "Scripts\Source\DNT_NorthCoastBoatParchmentPicker.psc"),
    (Join-Path $modRoot "Scripts\Source\DNT_NorthCoastBoatParchmentFragment.psc")
)
foreach ($requiredInput in $requiredInputs) {
    if (-not (Test-Path -LiteralPath $requiredInput -PathType Leaf)) {
        throw "Required North-coast package input not found: $requiredInput"
    }
    if ((Get-Item -LiteralPath $requiredInput).Length -le 0) {
        throw "North-coast package input is empty: $requiredInput"
    }
}
if ((Get-Item -LiteralPath (Join-Path $modRoot `
    "SEQ\DiegeticTravelBoatNorthCoast.seq")).Length -ne 4) {
    throw "North-coast SEQ must contain exactly one 4-byte FormID."
}
...
Write-Host "Packaged North-coast boat candidate: $archive"
