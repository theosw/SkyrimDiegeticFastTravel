...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required Lake Ilinalta Papyrus input not found: $requiredPath"
    }
}
...
$scripts = Get-ChildItem -LiteralPath $source -File `
    -Filter "DNT_IlinaltaBoat*.psc" | Sort-Object Name
if ($scripts.Count -ne 3) {
    throw "Expected exactly three Lake Ilinalta scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) Lake Ilinalta scripts to $output"
