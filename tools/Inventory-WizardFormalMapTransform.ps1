...
$inventoryRoot = Join-Path $buildRoot "wizard-formal-map-inventory"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "wizard-formal-map-transform.status"
$errorPath = Join-Path $buildRoot "wizard-formal-map-transform.error"
$reportPath = Join-Path $buildRoot "wizard-formal-map-transform.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryWizardFormalMapTransform.pas"
...
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required wizard formal-map inventory input not found: $required"
    }
}
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Wizard formal-map inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Wizard formal-map inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Wizard formal-map inventory did not create its report."
}
