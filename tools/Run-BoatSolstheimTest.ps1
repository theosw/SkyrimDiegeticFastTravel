param(
    ...
    [string]$BoatModName = "DiegeticTravel - Solstheim Boat Test",
    [string]$BoatPluginName = "DiegeticTravelBoatSolstheim.esp",
...
function Assert-TestReady {
    ...
    $requiredRuntime = @(
        ...
        (Join-Path $boatRoot "SEQ\DiegeticTravelBoatSolstheim.seq"),
        (Join-Path $boatRoot "Scripts\DNT_SolstheimBoatParchmentFragment.pex"),
        (Join-Path $boatRoot "Scripts\DNT_SolstheimBoatParchmentPicker.pex"),
        (Join-Path $boatRoot "Scripts\DNT_SolstheimBoatTravelService.pex"),
    ...
    foreach ($runtimeFile in $requiredRuntime) {
        if (-not (Test-Path -LiteralPath $runtimeFile -PathType Leaf)) {
            throw "Solstheim test runtime file was not found: $runtimeFile"
        }
    }
...
Assert-TestReady
Write-Output "Solstheim boat preflight passed for profile '$ProfileName'."
...
Write-Output "Monitoring fresh Solstheim boat and parchment output."
while ((Get-Date) -lt $deadline) {
    $skyrim = Get-Process -Id $skyrim.Id -ErrorAction SilentlyContinue
    if (-not $skyrim) {
        Write-Output "SkyrimSE exited. Observed $completionCount completed Solstheim trip(s)."
        exit 0
    }
    ...
    if (Test-Path -LiteralPath $LogPath -PathType Leaf) {
        ...
        if ($logFile.LastWriteTime -ge $launchTime) {
            ...
            if (-not $observedFreshPapyrus) {
                ...
                Write-Output "Fresh Papyrus log detected; Solstheim listener is live."
            }
            ...
            if ($lines.Count -gt $papyrusLineCount) {
                ...
                foreach ($line in $newLines) {
                    if ($line -match "\[DNT\].*lane=solstheim") {
                        Write-Output "SOLSTHEIM LOG: $line"
                        if ($line -match "BOAT_TRAVEL_COMPLETE") {
                            $completionCount += 1
                            Write-Output "SOLSTHEIM TRIP SUCCESS #${completionCount}"
                        }
                    } elseif (
                        $line -match "(?i)(DNT_Solstheim|lane=solstheim).*(error|warning)" -or
                        $line -match "(?i)(error|warning).*(DNT_Solstheim|lane=solstheim)"
                    ) {
                        Write-Warning "SOLSTHEIM SCRIPT ISSUE: $line"
                    }
                }
...
throw "Timed out after $TimeoutSeconds seconds. Observed $completionCount completed Solstheim trip(s)."
