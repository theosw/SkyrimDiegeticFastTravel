...
$inventoryRoot = Join-Path $buildRoot "dawnstar-morthal-wizard-spokes"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "dawnstar-morthal-wizard-spokes.status"
$errorPath = Join-Path $buildRoot "dawnstar-morthal-wizard-spokes.error"
$reportPath = Join-Path $buildRoot "dawnstar-morthal-wizard-spokes.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryDawnstarMorthalWizardSpokes.pas"
...
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required inventory input not found: $required"
    }
}
...
foreach ($inputName in @("Skyrim.esm", "Skyrim - Interface.bsa")) {
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required inventory input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Dawnstar/Morthal wizard inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Dawnstar/Morthal wizard inventory did not report success."
}
