...
& (Join-Path $PSScriptRoot "Inventory-BoatSolstheim.ps1") `
    -LoreRimRoot $LoreRimRoot `
    -XEdit $XEdit | Out-Host

if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Solstheim inventory report is missing."
}
$report = @(Get-Content -LiteralPath $reportPath)
$expectedActors = @(
    "KmodFerrymanSkaal",
    "KmodFerrymanRavenRock",
    "KmodFerrymanTelMithryn"
)
...
if ($sharedInfo.Count -ne 1) {
    throw "Solstheim shared ferry response does not match."
}
...
if (-not $found) {
    throw "Missing vanilla Solstheim shared-response FUZ: $expectedPath"
}
Write-Host "PASS voice asset $expectedPath"
Write-Host "PASS Solstheim ferryman voice audit complete"
