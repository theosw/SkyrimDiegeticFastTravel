...
$inventoryRoot = Join-Path $buildRoot "cfto-private-ferries-inventory"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "cfto-private-ferries-inventory.status"
$errorPath = Join-Path $buildRoot "cfto-private-ferries-inventory.error"
$reportPath = Join-Path $buildRoot "cfto-private-ferries-inventory.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryCFTOPrivateFerries.pas"
...
if (-not $resolvedInventory.StartsWith(
    ...
    throw "Refusing to clean CFTO private-ferry inventory outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required private-ferry inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required private-ferry inventory input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "CFTO private-ferry inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "CFTO private-ferry inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "CFTO private-ferry inventory did not create its report."
}
