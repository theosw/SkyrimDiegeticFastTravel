...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required Lake Honrich Papyrus input not found: $requiredPath"
    }
}

New-Item -ItemType Directory -Force -Path $output | Out-Null
$imports = "$source;$parchmentSource;$stubs"
$scripts = Get-ChildItem -LiteralPath $source -File -Filter "DNT_Boat*.psc" |
    Sort-Object Name
if ($scripts.Count -ne 3) {
    throw "Expected exactly three Lake Honrich scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) Lake Honrich scripts to $output"
