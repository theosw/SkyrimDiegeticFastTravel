...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required Baan Malur Papyrus input not found: $requiredPath"
    }
}
...
$scripts = Get-ChildItem -LiteralPath $source -File `
    -Filter "DNT_BaanMalurBoat*.psc" | Sort-Object Name
if ($scripts.Count -ne 3) {
    throw "Expected exactly three Baan Malur scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) Baan Malur scripts to $output"
