...
$inventoryRoot = Join-Path $buildRoot "boat-solstheim-inventory"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-solstheim-inventory.status"
$errorPath = Join-Path $buildRoot "boat-solstheim-inventory.error"
$reportPath = Join-Path $buildRoot "boat-solstheim-inventory.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryBoatSolstheim.pas"
...
if (-not $resolvedInventory.StartsWith(
    ...
    throw "Refusing to clean Solstheim inventory outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Solstheim inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required Solstheim inventory input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Solstheim inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Solstheim inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Solstheim inventory did not create its report."
}
