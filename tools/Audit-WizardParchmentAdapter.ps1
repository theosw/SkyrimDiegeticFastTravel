...
$auditRoot = Join-Path $buildRoot "wizard-parchment-audit"
$stagingData = Join-Path $auditRoot "data"
$pluginsList = Join-Path $auditRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "wizard-parchment-audit.status"
$errorPath = Join-Path $buildRoot "wizard-parchment-audit.error"
$reportPath = Join-Path $buildRoot "wizard-parchment-audit.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_AuditWizardParchmentAdapter.pas"
$seqPath = Join-Path $moduleRoot `
    "mod\SEQ\DiegeticTravelWizardParchment.seq"
...
if (-not $resolvedAuditRoot.StartsWith(
    ...
    throw "Refusing to clean wizard parchment audit outside build."
}

foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required wizard parchment audit input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required wizard parchment audit input not found: $inputPath"
    }
...
$plugins = @(
    ...
    (Join-Path $moduleRoot "mod\DiegeticTravelWizardParchment.esp")
)
foreach ($plugin in $plugins) {
    if (-not (Test-Path -LiteralPath $plugin -PathType Leaf)) {
        throw "Required wizard parchment audit plugin not found: $plugin"
    }
...
[System.IO.File]::WriteAllText(
    ...
    (
        ...
        "*DiegeticTravelWizardParchment.esp`r`n"
    ),
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Wizard parchment audit failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Wizard parchment audit did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Wizard parchment audit did not create its report."
}
...
if ($questIdLine.Count -ne 1) {
    throw "Wizard parchment audit does not identify one quest FormID."
}
if (-not (Test-Path -LiteralPath $seqPath -PathType Leaf)) {
    throw "Wizard parchment SEQ is missing: $seqPath"
}
$seqBytes = [System.IO.File]::ReadAllBytes($seqPath)
if ($seqBytes.Length -ne 4) {
    throw "Wizard parchment SEQ is not exactly one FormID."
}
...
if ($actualQuestId -ne $expectedQuestId) {
    throw ("Wizard parchment SEQ mismatch: expected {0:X8}, got {1:X8}" -f `
