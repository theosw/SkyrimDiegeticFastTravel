...
$generationRoot = Join-Path $buildRoot "boat-ilinalta-generation"
$stagingData = Join-Path $generationRoot "data"
$pluginsList = Join-Path $generationRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-ilinalta.status"
$errorPath = Join-Path $buildRoot "boat-ilinalta.error"
$scriptPath = Join-Path $PSScriptRoot "xedit\DNT_GenerateBoatIlinalta.pas"
$pluginOutput = Join-Path $modRoot "DiegeticTravelBoatIlinalta.esp"
$seqOutput = Join-Path $modRoot "SEQ\DiegeticTravelBoatIlinalta.seq"
$seqFormIdsPath = Join-Path $buildRoot "boat-ilinalta-seq-formids.txt"
...
if (-not $resolvedGenerationRoot.StartsWith(
    ...
    throw "Refusing to clean Lake Ilinalta staging outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Lake Ilinalta generation input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required Lake Ilinalta generation input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Lake Ilinalta generation failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Lake Ilinalta generation did not report success."
}
foreach ($outputPath in @($pluginOutput, $seqFormIdsPath)) {
    if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) {
        throw "Lake Ilinalta generation did not create: $outputPath"
    }
}
...
if ($seqFormIds.Count -ne 1 -or $seqFormIds[0] -notmatch "^[0-9A-Fa-f]{8}$") {
    throw "Lake Ilinalta generator did not report exactly one valid SEQ FormID."
}
...
Write-Host "Generated Lake Ilinalta boat candidate: $pluginOutput"
Write-Host "Generated Lake Ilinalta boat SEQ: $seqOutput ($($seqFormIds[0]))"
