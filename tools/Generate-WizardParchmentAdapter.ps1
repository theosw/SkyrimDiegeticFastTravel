...
$generationRoot = Join-Path $buildRoot "wizard-parchment-generation"
$stagingData = Join-Path $generationRoot "data"
$pluginsList = Join-Path $generationRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "wizard-parchment.status"
$errorPath = Join-Path $buildRoot "wizard-parchment.error"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_GenerateWizardParchmentAdapter.pas"
$pluginOutput = Join-Path $modRoot "DiegeticTravelWizardParchment.esp"
$seqOutput = Join-Path $modRoot "SEQ\DiegeticTravelWizardParchment.seq"
$seqFormIdsPath = Join-Path $buildRoot `
    "wizard-parchment-seq-formids.txt"
...
if (-not $resolvedGenerationRoot.StartsWith(
    ...
    throw "Refusing to clean wizard parchment staging outside build."
}

foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required wizard parchment generation input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required wizard parchment generation input not found: $inputPath"
    }
...
if (-not (Test-Path -LiteralPath $wizardPlugin -PathType Leaf)) {
    throw "Required core wizard plugin not found: $wizardPlugin"
}
Copy-Item -LiteralPath $wizardPlugin `
    -Destination (Join-Path $stagingData (Split-Path -Leaf $wizardPlugin)) `
    -Force
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Wizard parchment generation failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Wizard parchment generation did not report success."
}

foreach ($outputPath in @($pluginOutput, $seqFormIdsPath)) {
    if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) {
        throw "Wizard parchment generation did not create: $outputPath"
    }
}
...
if ($seqFormIds.Count -ne 1 -or $seqFormIds[0] -notmatch "^[0-9A-Fa-f]{8}$") {
    throw "Wizard parchment generator did not report one valid SEQ FormID."
}
...
Write-Host "Generated wizard parchment adapter: $pluginOutput"
Write-Host "Generated wizard parchment adapter SEQ: $seqOutput ($($seqFormIds[0]))"
