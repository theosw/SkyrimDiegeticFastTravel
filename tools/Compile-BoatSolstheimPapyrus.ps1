...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required Solstheim Papyrus input not found: $requiredPath"
    }
}
...
$scripts = Get-ChildItem -LiteralPath $source -File `
    -Filter "DNT_SolstheimBoat*.psc" | Sort-Object Name
if ($scripts.Count -ne 3) {
    throw "Expected exactly three Solstheim scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) Solstheim scripts to $output"
