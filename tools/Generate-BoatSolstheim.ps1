...
$generationRoot = Join-Path $buildRoot "boat-solstheim-generation"
$stagingData = Join-Path $generationRoot "data"
$pluginsList = Join-Path $generationRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-solstheim.status"
$errorPath = Join-Path $buildRoot "boat-solstheim.error"
$scriptPath = Join-Path $PSScriptRoot "xedit\DNT_GenerateBoatSolstheim.pas"
$pluginOutput = Join-Path $modRoot "DiegeticTravelBoatSolstheim.esp"
$seqOutput = Join-Path $modRoot "SEQ\DiegeticTravelBoatSolstheim.seq"
$seqFormIdsPath = Join-Path $buildRoot "boat-solstheim-seq-formids.txt"
...
if (-not $resolvedGenerationRoot.StartsWith(
    ...
    throw "Refusing to clean Solstheim staging outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Solstheim generation input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required Solstheim generation input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Solstheim generation failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Solstheim generation did not report success."
}
foreach ($outputPath in @($pluginOutput, $seqFormIdsPath)) {
    if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) {
        throw "Solstheim generation did not create: $outputPath"
    }
}
...
if ($seqFormIds.Count -ne 1 -or $seqFormIds[0] -notmatch "^[0-9A-Fa-f]{8}$") {
    throw "Solstheim generator did not report exactly one valid SEQ FormID."
}
...
Write-Host "Generated Solstheim boat candidate: $pluginOutput"
Write-Host "Generated Solstheim boat SEQ: $seqOutput ($($seqFormIds[0]))"
