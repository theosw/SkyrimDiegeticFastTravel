...
$inventoryRoot = Join-Path $buildRoot "solitude-wizard-spoke"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "solitude-wizard-spoke.status"
$errorPath = Join-Path $buildRoot "solitude-wizard-spoke.error"
$reportPath = Join-Path $buildRoot "solitude-wizard-spoke.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventorySolitudeWizardSpoke.pas"
...
if (-not $resolvedInventoryRoot.StartsWith(
    ...
    throw "Refusing to clean Solitude inventory directory outside build."
}

foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Solitude inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required Solitude inventory input not found: $inputPath"
    }
    Copy-Item -LiteralPath $inputPath `
        -Destination (Join-Path $stagingData $inputName) -Force
}

$bluePalacePlugin = Join-Path $LoreRimRoot `
    "mods\JK's Blue Palace\JK's Blue Palace.esp"
if (-not (Test-Path -LiteralPath $bluePalacePlugin -PathType Leaf)) {
    throw "JK's Blue Palace plugin was not found: $bluePalacePlugin"
}
Copy-Item -LiteralPath $bluePalacePlugin `
    -Destination (Join-Path $stagingData "JK's Blue Palace.esp") -Force

[System.IO.File]::WriteAllText(
    $pluginsList,
    "*JK's Blue Palace.esp`r`n",
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Solitude wizard-spoke inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Solitude wizard-spoke inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Solitude wizard-spoke inventory did not create its report."
}
