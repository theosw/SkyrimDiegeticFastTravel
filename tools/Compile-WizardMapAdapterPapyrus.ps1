...
$wizardSource = Join-Path $projectRoot `
...
foreach ($requiredPath in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required wizard map-adapter build input not found: $requiredPath"
    }
}
...
$scripts = Get-ChildItem -LiteralPath $source -File `
    -Filter "DNT_WizardMap*.psc" | Sort-Object Name
if ($scripts.Count -ne 2) {
    throw "Expected exactly two wizard map-adapter scripts, found $($scripts.Count)."
}
...
Write-Host "Compiled $($scripts.Count) wizard map-adapter scripts to $output"
