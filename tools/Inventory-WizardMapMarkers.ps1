...
$inventoryRoot = Join-Path $buildRoot "wizard-map-markers"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "wizard-map-markers.status"
$errorPath = Join-Path $buildRoot "wizard-map-markers.error"
$reportPath = Join-Path $buildRoot "wizard-map-markers.report.txt"
$scriptPath = Join-Path $PSScriptRoot "xedit\DNT_InventoryWizardMapMarkers.pas"
...
if (-not $resolvedInventoryRoot.StartsWith(
    ...
    throw "Refusing to clean wizard map-marker inventory outside build."
}

foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required wizard map-marker inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required wizard map-marker inventory input not found: $inputPath"
    }
...
if (-not (Test-Path -LiteralPath $bcdPlugin -PathType Leaf)) {
    throw "Better Carriage Destinations plugin was not found: $bcdPlugin"
}
Copy-Item -LiteralPath $bcdPlugin `
    -Destination (Join-Path $stagingData "Better Carriage Destinations.esp") `
    -Force
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Wizard map-marker inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Wizard map-marker inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Wizard map-marker inventory did not create its report."
}
