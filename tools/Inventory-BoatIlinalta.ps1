...
$inventoryRoot = Join-Path $buildRoot "boat-ilinalta-inventory"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-ilinalta-inventory.status"
$errorPath = Join-Path $buildRoot "boat-ilinalta-inventory.error"
$reportPath = Join-Path $buildRoot "boat-ilinalta-inventory.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryBoatIlinalta.pas"
...
if (-not $resolvedInventory.StartsWith(
    ...
    throw "Refusing to clean Lake Ilinalta inventory outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Lake Ilinalta inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required Lake Ilinalta inventory input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "Lake Ilinalta inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "Lake Ilinalta inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Lake Ilinalta inventory did not create its report."
}
