unit DNT_GenerateBoatSolstheim;

uses SysUtils, Classes;

const
  OutputPluginName = 'DiegeticTravelBoatSolstheim.esp';
...
begin
  ...
  if CurrentObjectID > StartObjectID then
    raise Exception.Create(
      'Solstheim FormID range is already occupied: ' +
...
begin
  ...
  if not Assigned(TopicRecord) or not Assigned(BranchRecord) then
    raise Exception.Create('Could not clone Solstheim dialogue shell');

  SetElementEditValues(TopicRecord, 'EDID', 'DNT_BoatSolstheimParchmentTopic');
  SetElementEditValues(TopicRecord, 'FULL', BoatPrompt);
  SetElementEditValues(BranchRecord, 'EDID', 'DNT_BoatSolstheimParchmentBranch');
  ...
  SetElementEditValues(InfoRecord, 'EDID', 'DNT_BoatSolstheimParchmentInfo');
...
begin
  ...
  StatusPath := ScriptsPath + '..\..\build\boat-solstheim.status';
  ErrorPath := ScriptsPath + '..\..\build\boat-solstheim.error';
  if ReleaseMode then begin
    PluginOutputPath := ScriptsPath +
      '..\..\build\release\DiegeticTravel.esp';
    SeqFormIDsPath := ScriptsPath +
      '..\..\build\release-boat-solstheim-seq-formids.txt';
  end else begin
    PluginOutputPath := ScriptsPath +
      '..\..\modules\boat-solstheim\mod\DiegeticTravelBoatSolstheim.esp';
    SeqFormIDsPath := ScriptsPath +
      '..\..\build\boat-solstheim-seq-formids.txt';
  end;
  ...
  try
    ...
    BoatQuest := NewQuest('DNT_BoatSolstheimQuest');
    ServiceScript := AddScript(BoatQuest, 'DNT_SolstheimBoatTravelService');
    PickerScript := AddScript(BoatQuest, 'DNT_SolstheimBoatParchmentPicker');
    ...
    AddMessage('[DNT] Generated Solstheim boat parchment candidate');
  except
