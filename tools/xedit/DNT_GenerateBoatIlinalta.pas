unit DNT_GenerateBoatIlinalta;

uses SysUtils, Classes;

const
  OutputPluginName = 'DiegeticTravelBoatIlinalta.esp';
...
begin
  ...
  if CurrentObjectID > StartObjectID then
    raise Exception.Create(
      'Lake Ilinalta FormID range is already occupied: ' +
...
begin
  ...
  if not Assigned(TopicRecord) or not Assigned(BranchRecord) then
    raise Exception.Create('Could not clone Lake Ilinalta dialogue shell');

  SetElementEditValues(TopicRecord, 'EDID', 'DNT_BoatIlinaltaParchmentTopic');
  SetElementEditValues(TopicRecord, 'FULL', BoatPrompt);
  SetElementEditValues(BranchRecord, 'EDID', 'DNT_BoatIlinaltaParchmentBranch');
  ...
  SetElementEditValues(InfoRecord, 'EDID', 'DNT_BoatIlinaltaParchmentInfo');
  ...
  FragmentScript := ConfigureInfoFragment(
    InfoRecord,
    TemplateInfo,
    'DNT_IlinaltaBoatParchmentFragment'
  );
...
begin
  ...
  StatusPath := ScriptsPath + '..\..\build\boat-ilinalta.status';
  ErrorPath := ScriptsPath + '..\..\build\boat-ilinalta.error';
  if ReleaseMode then begin
    PluginOutputPath := ScriptsPath +
      '..\..\build\release\DiegeticTravel.esp';
    SeqFormIDsPath := ScriptsPath +
      '..\..\build\release-boat-ilinalta-seq-formids.txt';
  end else begin
    PluginOutputPath := ScriptsPath +
      '..\..\modules\boat-ilinalta\mod\DiegeticTravelBoatIlinalta.esp';
    SeqFormIDsPath := ScriptsPath +
      '..\..\build\boat-ilinalta-seq-formids.txt';
  end;
  ...
  try
    ...
    ProviderWhitelist := NewFormList('DNT_BoatIlinaltaProviders');
    AddFormListEntry(ProviderWhitelist, RequireRecord(
      CftoFile, $0332F5, 'NPC_', 'KmodFerrymanGuardian'
    ));
    AddFormListEntry(ProviderWhitelist, RequireRecord(
      CftoFile, $0332ED, 'NPC_', 'KmodFerrymanHalfMoon'
    ));
    AddFormListEntry(ProviderWhitelist, RequireRecord(
      CftoFile, $02E1E6, 'NPC_', 'KmodFerrymanBrittleshin'
    ));
    AddFormListEntry(ProviderWhitelist, RequireRecord(
      CftoFile, $014C80, 'NPC_', 'KmodFerrymanLakeview'
    ));

    BoatQuest := NewQuest('DNT_BoatIlinaltaQuest');
    ServiceScript := AddScript(BoatQuest, 'DNT_IlinaltaBoatTravelService');
    PickerScript := AddScript(BoatQuest, 'DNT_IlinaltaBoatParchmentPicker');
    ...
    AddMessage('[DNT] Generated Lake Ilinalta boat parchment candidate');
  except
