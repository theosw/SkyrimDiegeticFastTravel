...
begin
  ...
  ServiceScript := ScriptByName(BoatQuest, 'DNT_BoatTravelService');
  PickerScript := ScriptByName(BoatQuest, 'DNT_BoatParchmentPicker');
...
begin
  ...
  BoatBranch := RequireBoatRecord(
    'DLBR',
    'DNT_BoatHonrichParchmentBranch'
  );
  BoatTopic := RequireBoatRecord(
    'DIAL',
    'DNT_BoatHonrichParchmentTopic'
  );
  ...
  if GetElementEditValues(BoatInfo, 'EDID') <>
    'DNT_BoatHonrichParchmentInfo' then
  ...
  if not Assigned(VMAD) or
    ...
    (GetElementEditValues(VMAD, 'Script Fragments\FileName') <>
      'DNT_BoatParchmentFragment') then
...
begin
  Result := 1;
  StatusPath := ScriptsPath + '..\..\build\boat-honrich-audit.status';
  ErrorPath := ScriptsPath + '..\..\build\boat-honrich-audit.error';
  ReportPath := ScriptsPath +
    '..\..\build\boat-honrich-audit.report.txt';
  WriteTextFile(StatusPath, 'running');
  ReportLines := TStringList.Create;
  try
    BoatFile := FileByPluginName('DiegeticTravelBoatHonrich.esp');
    ...
    AuditQuest;
    AuditDialogue;
    ReportLines.Add('PASS Lake Honrich boat plugin audit complete');
