unit DNT_GenerateBoatHonrich;

uses SysUtils, Classes;

const
  OutputPluginName = 'DiegeticTravelBoatHonrich.esp';
  BoatPrompt = 'Could you show me the Lake Honrich route?';
...
begin
  ...
  if not Assigned(TopicRecord) or not Assigned(BranchRecord) then
    raise Exception.Create('Could not clone Lake Honrich dialogue shell');

  SetElementEditValues(TopicRecord, 'EDID', 'DNT_BoatHonrichParchmentTopic');
  SetElementEditValues(TopicRecord, 'FULL', BoatPrompt);
  SetElementEditValues(BranchRecord, 'EDID', 'DNT_BoatHonrichParchmentBranch');
  ...
  SetElementEditValues(InfoRecord, 'EDID', 'DNT_BoatHonrichParchmentInfo');
...
begin
  ...
  try
    ...
    AddMessage('[DNT] Generated Lake Honrich boat parchment candidate');
  except
