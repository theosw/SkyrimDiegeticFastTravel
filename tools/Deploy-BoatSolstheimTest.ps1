param(
    ...
    [string]$DeploymentMod = "DiegeticTravel - Solstheim Boat Test"
)

$ErrorActionPreference = "Stop"

if (Get-Process SkyrimSE -ErrorAction SilentlyContinue) {
    throw "Close SkyrimSE before deploying the Solstheim boat test mod."
}

$projectRoot = Split-Path -Parent $PSScriptRoot
$sourceRoot = Join-Path $projectRoot "modules\boat-solstheim\mod"
if (-not (Test-Path -LiteralPath $sourceRoot -PathType Container)) {
    throw "Solstheim boat module was not found: $sourceRoot"
}
...
$ownershipMarker = Join-Path $targetRoot "DNT_BOAT_SOLSTHEIM_TEST_OWNED.txt"
if (Test-Path -LiteralPath $targetRoot) {
    ...
    Set-Content -LiteralPath $ownershipMarker -Encoding UTF8 -Value @(
        "Owned local development mod for Diegetic Travel Solstheim boat tests."
...
$expectedRelativePaths = @(
    ...
    "README-BoatSolstheim.txt",
    "SEQ\DiegeticTravelBoatSolstheim.seq",
    "Scripts\DNT_SolstheimBoatParchmentFragment.pex",
    "Scripts\DNT_SolstheimBoatParchmentPicker.pex",
    "Scripts\DNT_SolstheimBoatTravelService.pex",
    "Scripts\Source\DNT_SolstheimBoatParchmentFragment.psc",
    "Scripts\Source\DNT_SolstheimBoatParchmentPicker.psc",
    "Scripts\Source\DNT_SolstheimBoatTravelService.psc"
)
foreach ($relativePath in $expectedRelativePaths) {
    $source = Join-Path $sourceRoot $relativePath
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
        throw "Required Solstheim runtime file is missing: $source"
    }
    ...
    if (-not $destination.StartsWith(
        ...
        throw "Unsafe Solstheim deployment path: $destination"
    }
...
$allowedFiles = @(
    $expectedRelativePaths + "DNT_BOAT_SOLSTHEIM_TEST_OWNED.txt"
)
...
Write-Host "Deployed isolated Solstheim boat test mod: $targetRoot"
