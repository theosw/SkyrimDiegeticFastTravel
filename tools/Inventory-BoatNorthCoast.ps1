...
$inventoryRoot = Join-Path $buildRoot "boat-north-coast-inventory"
$stagingData = Join-Path $inventoryRoot "data"
$pluginsList = Join-Path $inventoryRoot "plugins.txt"
$statusPath = Join-Path $buildRoot "boat-north-coast-inventory.status"
$errorPath = Join-Path $buildRoot "boat-north-coast-inventory.error"
$reportPath = Join-Path $buildRoot `
    "boat-north-coast-inventory.report.txt"
$scriptPath = Join-Path $PSScriptRoot `
    "xedit\DNT_InventoryBoatNorthCoast.pas"
...
if (-not $resolvedInventory.StartsWith(
    ...
    throw "Refusing to clean north-coast inventory outside build."
}
foreach ($required in @($xeditPath, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required north-coast inventory input not found: $required"
    }
}
...
foreach ($inputName in @(
    ...
    if (-not (Test-Path -LiteralPath $inputPath -PathType Leaf)) {
        throw "Required north-coast inventory input not found: $inputPath"
    }
...
if ($terminalStatus -eq "failed") {
    ...
    throw "North-coast inventory failed: $detail"
}
if ($terminalStatus -ne "success") {
    throw "North-coast inventory did not report success."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "North-coast inventory did not create its report."
}
